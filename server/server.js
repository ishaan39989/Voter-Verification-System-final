    ...
    try {
        ...
        if (!voterId || typeof voterId !== 'string') {
            return res.status(400).json({
                success: false,
                message: 'Voter ID is required and must be a string'
            });
        }
        
        if (!facePhoto || typeof facePhoto !== 'string' || !facePhoto.includes('base64')) {
            return res.status(400).json({
                success: false,
                message: 'Face photo is required and must be a valid base64 string'
            });
        }
